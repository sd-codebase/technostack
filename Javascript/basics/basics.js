use strict
    ...
    strict mode will start from line where we have written "use start"
    Strict mode makes it easier to write "secure" JavaScript.
    eg
    x = 3.14;       // This will cause an error because x is not declared
    deletng variable or object or function is not allowed
...
Dynamic Language
    ...
    Eg. In javascript, we declare variable and assign number, and after we can assign string or any other type values;
...
Prototype
    prototype is a property which has value object associated with every function and object in JavaScript
    functions prototype prop is accesible and we can modify
    ...
    When we create an object, __proto__ property gets added to instance,
     this property points to constructor's prototype property
     Means in other words when we access __proto__ property it will give us prototype proprty of constructor
...
Prototype chain
    Javascript engine check that if given ptoperty or method is attached to instance or object or not, if found returns
    In not, it goes to __proto__ property of object, and check there if found returns, this __proto__ proprty points to the parent's prototype property.
    If not, goes again to prototype property of parent, it goes till Object function because everything is derieved from Object function.
    Lastly if the property does not find in prototype property of Object, it will return null
    This is called prototype chain
    eg, D is object which is created by C, which extends to B, which extends to A.
    so when we try to access someproperty it goes till Object functions prototype property;

Inheritance using Prototype
    Inheritance is important concept when we works with objects, we can reuse functionality in inheritance
    ...
    1- Declare Base class, 2- attach methor to prototype property of Person, so children of person can use them
    3- declare child class, 4- Give call to Parent similar to super() in other languages
    5- point prototype of Student to Person class by creating Object of Person
    6- point prototype.constructor to child
    7- Create object of Student; Now we can ascess method from base class also



hasOwnProperty()
    The hasOwnProperty() method returns a boolean indicating whether the object has the specified property as its own property and not inherited from parent




Hoisting
    Hoisting in javascript means move the declaration of functions, variables or classes to the top of their scope, before execution of the code.
    let and const doen't get hoisted and returns ReferenceError if used before declaration
    Initializations are Not Hoisted just declaration hoisted
...
Scope
    Scope is area where an variable is accessible to.
    Scope is current context of execution, 
    Scope is the context inwhich values and expressions are visible or can be referenced
...
Closures
    Closure is the combination of a function, bundled together with references to its surrounding state i.e. the lexical environment.
    In other words, a closure gives you access to an outer function's scope from an inner function.
    We can use private variables using closures,
    We can hide internal implemetation fron users
    function add() {
        let counter = 0;
...
this keyword
    ...
    5. Methods like call(), apply(), and bind() can refer this to passed object while calling function with call and apply and passed object while creating function using bind.
...
Curry and use cases
    in functional programming, transformation of the function of multiple arguments into several functions of a single argument in sequence.
    ...
    Use Case
    It lets you compose the sequence of your functions and ensures that the particular sequence is enforced and followed.
...
debouncing
    ...
    (child asking for icecream and mom says I'll give an icecreame only if you keeps asking)
...
Callbacks,
    When we pass a function as argument to a function this type of argument is called as Callback function
    Eg
...
Execution Context
    The Execution Context contains the code that's currently running, and everything that used in its execution i.e. variables and functions.
    During the Execution Context run-time,
     the specific code gets parsed by a parser, the variables and functions are stored in memory, executable byte-code gets generated, and the code gets executed.
...
Execution Stack or Call Stack
    ...
    when other actions, functions, and events occur, an Execution Context is created for each of these events
...
Event loop
    ...
    Which is responsible for executing the code, collecting events and processing those events, and executing queued sub-tasks
    1. Heap - stores variables needed in execition
    2. Call Stack - when execution contexts get created, those will add to the stack,
    3. Web or Browser Api - callbacks wait for web api's, async operations to complete or timers to complete
    4. Callback or Event queue - Once the timeres and async operations complete callbacks are get pushed to callback queue, or event queue
        - Macrotasks queue - setInterval, setTimeout callback functions
        - Microtasks queus - resolve, reject, finally handler of promises, or callback function of queueMicrotask() function
...
Macro Tasks and Micro tasks
    ...
    Immediately after every macrotask, the engine executes all tasks from microtask queue. before start execution of macrotask
    Eg
    setTimeout(func1); Promise.resolve().then(func2);Promise.resolve().then(func3);
    func2, and func3 will executes first and then func1 will executes; becuase func2, func3 gets added to microtask queue



Event Deligation
    We can listen events only on those element which are existed in page; but there is problem if elements get loaded dynalically after registering event listners
    We can register event listernes on parent elements, and listen events of dynamically loaded elements, in this way we can solve the above issue.
    This is called event delegation model.
    In this we just add event listener on ul element and listens the event generated on li element also. we can find source element using event.target property
...
Bubbling And Capturing
    When we generate event on any element, that same event triggers on all parent elements till the html element.
    When events generates from child to parent this is called as event Bubbling,
    And when event first generates on html element and then for children till the source element (which element generated element) this is called as Capturing
    Bubbling is happenes by default, to make it capture we pass third argument false to addEventListener function
...
Promises
    ...
    Promise.resolve(new Error('fail')).then(resolved, rejected); // always resolve will call
    - Promise.all()
    returns a new Promise(). returned new promise resolves if all input promises resolves else rejects immidiately if one or the promise rejects
    - Promise.allSettled()
    returns a new Promise(). returned new promise resolves when all input promises settles (reject or fullfile)
    - Promise.any()
    returns a new Promise(). returned new promise resolves if any one of input promises resolves, if not single promise resolves, returned promise rejects
    - Promise.race()
    returns a new Promise(). returned new promise resolves or rejects if one of the promise resolves or rejects
...
let vs const vs var
    ...
    3.  var variables are hoisted at the top of the scope and initiatilizes to undefined
        let and const are hoisted at the top of the scope but it doesn't getinitialized,
         that is why if we try to access let or const variable before declaration it gives ReferenceError
...
Reverse string
    ...
    3. recursion
        reverseString(str) {
            return (str === '') ? '' : reverseString(sustr(1)) + str.charAt(0);
        }
        reverseString('hello');
